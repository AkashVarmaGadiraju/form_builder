...
import CreateFieldGroupModal from "../../components/main-componentts/CreateFieldGroupModal";
import {
	Button,
	Dropdown,
	DropdownItem,
...
const FieldGroup: FunctionComponent = () => {
	const [groups, setGroups]: [any[], any] = React.useState([]);
	...
	const handleDeleteField = () => {
		setGroups(groups.filter((field) => field?.id !== selectedField?.id));
	};
			...
			case "fields":
				...
				if (field.selectedFields) {
					...
					for (let category in field.selectedFields) {
						if (field.selectedFields[category]) {
							categories.push(field.selectedFields[category]?.name);
						}
					}
				}
				return (
					<div className="flex flex-row gap-3 max-w-[100%] w-full overflow-x-scroll flex-nowrap">
						{categories.map((e: string) => (
							<div className="border-[#B7EB8F] bg-[#F6FFED] border-1 px-2 py-1 rounded-md ">
								<p className="text-[#52C41A] whitespace-nowrap">{capitalize(e)}</p>
							</div>
						))}
					</div>
					...
					<form className="m-0 self-stretch h-[55px] flex flex-row items-start justify-start gap-[9px] max-w-full">
						...
						<div className="flex flex-row justify-end gap-[9px]">
							...
							<div className="h-full w-[162.4px] flex flex-col items-start justify-start px-0 pb-0 box-border">
								<CreateButton
									title={"Create Group"}
									sheetContent={<CreateFieldGroupModal groups={groups} setGroups={setGroups} />}
									sheetContentClasses="overflow-hidden !w-[500px] !max-w-[600px]"
								/>
							</div>
						</div>
					</form>
