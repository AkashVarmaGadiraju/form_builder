...
import { cn } from "../util-components/utils";
  ...
  return (
    <div className="absolute w-[81px] bg-lavender box-border overflow-hidden shrink-0 flex flex-col items-center justify-between pt-[25px] px-0 pb-[33px] text-left text-4xs text-steelblue font-monoton border-[1px] border-solid border-cornflowerblue mq825:pb-5 mq825:box-border mq1250:pt-5 mq1250:pb-[21px] mq1250:box-border h-full">
      <div className="flex flex-col items-center justify-start gap-[1.6px] flex-shrink-0">
        <img
          className="w-[48px] h-[50.4px] relative object-cover z-[1]"
          loading="lazy"
          alt=""
          src="/forrmlogo-1@2x.png"
        />
        <div className="flex flex-row items-start justify-center py-0 pr-0.5 pl-[3px]">
          <div className="relative inline-block min-w-[33px] z-[1] text-center">
            NOVA
          </div>
        </div>
      </div>
      <div className="flex flex-col justify-end items-end gap-[18px] flex-grow">
        ...
        >
          <FieldIcon
            className={cn(
              "w-[30.9px] h-6 relative",
              currentRoute === "fields" ? "fill-[#3BA0AE]" : "fill-[#7d97bc]"
            )}
          />
        </Button>
        <Button
        ...
        >
          <div className="flex flex-row items-start justify-end">
            <GroupIcon
              className={cn(
                "h-7 w-[27px] relative",
                currentRoute === "groups" ? "fill-[#3BA0AE]" : "fill-[#7d97bc]"
              )}
            />
          </div>
        </Button>
        ...
        >
          <div className="flex flex-row items-start justify-end">
            <TemplateIcon
              className={cn(
                "h-[26px] w-[21px] relative",
                currentRoute === "models" ? "fill-[#3BA0AE]" : "fill-[#7d97bc]"
              )}
            />
          </div>
        </Button>
        ...
        >
          <div className="flex flex-row items-start justify-end">
            <SettingsIcon
              className={cn(
                "h-6 w-[23px] relative",
                currentRoute === "forms" ? "fill-[#3BA0AE]" : "fill-[#7d97bc]"
              )}
            />
          </div>
        </Button>
